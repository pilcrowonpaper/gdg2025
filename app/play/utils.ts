...
export class Renderer {
	...
	public pushSprite(spritePixels: Uint8Array, x: number, y: number): void {
		...
		for (const queueItem of this.renderQueue) {
			...
			for (let i = 0; i < 16 - spriteLeftOffset - spriteRightOffset; i++) {
				for (let j = 0; j < 16 - spriteTopOffset - spriteBottomOffset; j++) {
					const pixelPosition = graphics.getSpritePixelPositionFromCoordinates({
						x: spriteLeftOffset + i,
						y: spriteTopOffset + j,
					});
