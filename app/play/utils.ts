...
export class Renderer {
	...
	public pushSprite(spritePixels: Uint8Array, x: number, y: number, flipX: boolean, flipY: boolean): void {
	...
	public render(): void {
		...
		for (const queueItem of this.renderQueue) {
			...
			if (canvasXEnd > this.canvasContext.canvas.width) {
				spriteRightOffset = canvasXEnd - this.canvasContext.canvas.width;
			}
			let spriteBottomOffset = 0;
			if (canvasYEnd > this.canvasContext.canvas.height) {
				spriteBottomOffset = canvasYEnd - this.canvasContext.canvas.height;
			}

			if (spriteLeftOffset >= 16 || spriteTopOffset >= 16 || spriteRightOffset >= 16 || spriteBottomOffset >= 16) {
				continue;
			}

			for (let i = 0; i < 16 - spriteLeftOffset - spriteRightOffset; i++) {
				for (let j = 0; j < 16 - spriteTopOffset - spriteBottomOffset; j++) {
					...
					const pixelPosition = graphics.getSpritePixelPositionFromCoordinates(spriteX, spriteY);
					const color = graphics.getPixelColor(queueItem.spritePixels, pixelPosition);
					if (color !== null) {
						const index = (renderStartX + i + (renderStartY + j) * this.canvasContext.canvas.width) * 4;
						if (queueItem.colorId !== null) {
							const color = graphics.resolvePixelColor(queueItem.fullColor, queueItem.colorId);
