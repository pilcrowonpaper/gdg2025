...
async function init(): Promise<void> {
    ...
    // updateInstructionsExternalFunctions.set("fill", (args) => {
    // 	// fill("sprite", true, true, 3, 5, 5)
    // 	if (args.length !== 5) {
    // 		const result: puffin.ExternalFunctionErrorResult = {
    // 			ok: false,
    ...
    updateInstructionsExternalFunctions.set("set_background_pixels", (args) => {
        ...
        const backgroundColor = graphics.resolvePixelColor(
            solidColor,
            fullColor,
            colorId
        );
...
async function executeUpdateInstructions(
    ...
    const executeResult = puffin.executeInstructions(
        parsedUpdateScript.instructions,
        state,
        externalFunctions
    );
    if (!executeResult.ok) {
        writeExecutionError(
            parsedUpdateScript.id,
...
function writeScriptParseError(
    scriptId: string,
    script: string,
    errorResult: puffin.ParseErrorResult
): void {
    const chars = puffin.parseString(script);
    const lineColumnPosition = getLineColumnPosition(
        chars,
        errorResult.position
    );
    ...
    writeToOutput(
        `${errorResult.message} (line ${lineColumnPosition.line + 1}, column ${
    ...
    writeToOutput(
        String.fromCodePoint(
            ...lineChars.subarray(0, lineColumnPosition.column)
        ),
        {
            color: "gray",
            textDecoration: "none",
    ...
    writeToOutput(
        String.fromCodePoint(
            ...lineChars.subarray(lineColumnPosition.column + 1)
        ),
        {
            color: "gray",
            textDecoration: "none",
...
function writeExecutionError(
    scriptId: string,
    script: string,
    errorResult: puffin.ExecutionErrorResult
): void {
    const chars = puffin.parseString(script);
    const startLineColumnPosition = getLineColumnPosition(
        chars,
        errorResult.startPosition
    );
    const endLineColumnPosition = getLineColumnPosition(
        chars,
        errorResult.endPosition
    );
    ...
    writeToOutput(
        `${errorResult.message} (line ${
            startLineColumnPosition.line + 1
        }, column ${startLineColumnPosition.column + 1}-${
    ...
    writeToOutput(
        String.fromCodePoint(
            ...lineChars.subarray(0, startLineColumnPosition.column)
        ),
        {
            color: "gray",
            textDecoration: "none",
    ...
    writeToOutput(
        String.fromCodePoint(
            ...lineChars.subarray(
                startLineColumnPosition.column,
                endLineColumnPosition.column
            )
        ),
        {
    ...
    writeToOutput(
        String.fromCodePoint(
            ...lineChars.subarray(endLineColumnPosition.column)
        ),
        {
            color: "gray",
            textDecoration: "none",
...
function getLineColumnPosition(
    chars: Uint32Array,
    position: number
): LineColumnPosition {
...
function writeToOutput(
    message: string,
    attributes: OutputTextAttributes
): void {
