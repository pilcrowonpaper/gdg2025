...
function setupColorPicker(): void {
    ...
    for (let i = 0; i < graphics.colors.length; i++) {
        ...
        input.style.backgroundColor = graphics.colorToHexCode(
            graphics.colors[i]
        );
        if (i === 0) {
            input.checked = true;
...
function handleDraw(e: PointerEvent): void {
    ...
    const x = Math.floor(
        (e.clientX - rect.left) / (rect.width / graphics.spriteSize)
    );
    const y = Math.floor(
        (e.clientY - rect.top) / (rect.height / graphics.spriteSize)
    );
...
function bucketFill(
    ...
    while (positionStack.length > 0) {
        ...
        if (
            curr.solid !== targetSolid ||
            curr.full !== targetFull ||
            curr.colorId !== targetIndex
        ) {
            continue;
        }
        ...
        const { x, y } =
            graphics.getSpriteCoordinatesFromPixelPosition(position);
        if (x > 0) {
...
function setupToolTypeSelector(): void {
    getToolTypeSelectorElement().addEventListener("change", () =>
        redrawCurrentSprite()
    );
}
...
function setupPageToggle(): void {
    const editBtn = document.getElementById(
        "show-sprite-edit-page-button"
    ) as HTMLButtonElement;
    const configBtn = document.getElementById(
        "show-sprite-config-page-button"
    ) as HTMLButtonElement;
