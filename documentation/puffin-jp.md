...
### 要素の追加
 ...
 後述する `add` 関数を使って、リストやオブジェクトの末尾に要素を足すこともできます。

`do @add list, "foo"`   
`do @add object, {x: 3, y: 5}`
...
| 関数名 | 説明 |
| ----- | ----- |
| `@add target, value` | リストやオブジェクトに要素を足す ※Pythonのリストに対するappendのようなもの |
...
`if` は「もし〜なら〜する」です。  
 ...
 インデント（字下げ）には **タブ** を使います。  
 スペースは使えないことに注意してください。
