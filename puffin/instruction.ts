...
export function parseInstructions(
    ...
    while (true) {
        ...
        let emptyLineSize = 0;
        while (true) {
            if (start + resultSize + emptyLineSize >= chars.length) {
                emptyLine = true;
                break;
            }
            if (chars[start + resultSize + emptyLineSize ] === CODE_POINT_TAB) {
                emptyLineSize++;
                continue;
            }
            if (chars[start + resultSize + emptyLineSize ] === CODE_POINT_NEWLINE) {
                emptyLineSize++;
				emptyLine = true
				break
            }
        ...
        const parseInstructionResult = parseInstruction(
            chars,
            start + resultSize,
            level
        );
...
export type ParseInstructionsResult =
    | ParseInstructionsSuccessResult
    | ParseErrorResult;
...
function parseInstruction(
    ...
    const parseInstructionNameResult = parseInstructionName(
        chars,
        start + resultSize
    );
    ...
    if (instructionName === "do") {
        ...
        const parseExpressionResult = parseExpression(
            chars,
            start + resultSize
        );
    ...
    if (instructionName === "set") {
        ...
        const parseInstructionTargetResult = parseInstructionTarget(
            chars,
            start + resultSize
        );
        ...
        const parseVariableValueResult = parseExpression(
            chars,
            start + resultSize
        );
    ...
    if (instructionName === "if") {
        ...
        while (true) {
            const parseConditionResult = parseExpression(
                chars,
                start + resultSize
            );
        ...
        if (start + resultSize < chars.length) {
            ...
            const parseInstructionsResult = parseInstructions(
                chars,
                start + resultSize,
                level + 1
            );
    ...
    if (instructionName === "for") {
        ...
        const parseLoopVariableNameResult = parseIdentifier(
            chars,
            start + resultSize
        );
...
function parseInstructionTarget(
    chars: Uint32Array,
    start: number
): ParseInstructionTargetResult {
    let resultSize = 0;
    ...
    while (true) {
        ...
        if (chars[start + resultSize] === CODE_POINT_PERIOD) {
            const parsePropertyAccessorResult =
                parsePropertyAccessorInstructionTargetModifier(
                    chars,
                    start + resultSize
                );
            ...
            const parseIndexAccessorResult =
                parseIndexAccessorInstructionTargetModifier(
                    chars,
                    start + resultSize
                );
...
type ParseInstructionTargetResult =
    | ParseInstructionTargetSuccessResult
    | ParseErrorResult;
...
function parseIndexAccessorInstructionTargetModifier(
    ...
    const parseIndexExpressionResult = parseExpression(
        chars,
        start + resultSize
    );
...
function parseInstructionName(
    ...
    const instructionName = String.fromCharCode(
        ...Array.from(chars.slice(start, start + resultSize))
    );
...
type ParseInstructionNameResult =
    | ParseInstructionNameSuccessResult
    | ParseErrorResult;
