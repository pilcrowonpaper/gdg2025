...
export type ExecuteInstructionResult =
        ...
        switch (instruction.type) {
            case "instruction.set": {
                ...
                if (instruction.targetNode.modifiers.length < 1) {
                    const resolveValueResult = resolveExpression(
                        instruction.valueNode,
                        memory,
                        externalFunctions
                    );
                ...
                const variableValue =
                    memory.get(instruction.targetNode.variableName) ?? null;
                ...
                const getReferenceResult =
                    getValueReferenceFromInstructionTargetModifiers(
                        copiedVariableValue,
                ...
                const lastModifier =
                    instruction.targetNode.modifiers[
                        instruction.targetNode.modifiers.length - 1
                    ];
                switch (lastModifier.type) {
                    case "instruction_target_modifier.index_accessor": {
                        ...
                        const resolveIndexResult = resolveExpression(
                            lastModifier.indexExpressionNode,
                            memory,
                            externalFunctions
                        );
                    ...
                    case "instruction_target_modifier.property_accessor": {
                        ...
                        reference.properties.set(
                            lastModifier.propertyName,
                            resolveValueResult.value
                        );
                        break;
                    }
                }
                ...
                memory.set(
                    instruction.targetNode.variableName,
                    copiedVariableValue
                );
                break;
            }
            ...
            case "instruction.do": {
                ...
                const resolveExpressionResult = resolveExpression(
                    instruction.expressionNode,
                    memory,
                    externalFunctions
                );
            ...
            case "instruction.add": {
                ...
                const resolveItemValueResult = resolveExpression(
                    instruction.valueNode,
                    memory,
                    externalFunctions
                );
                ...
                const copiedItemValue = structuredClone(
                    resolveItemValueResult.value
                );
            ...
            case "instruction.if": {
                ...
                for (const conditionNode of instruction.conditionNodes) {
                    const resolveConditionValueResult = resolveExpression(
                        conditionNode,
                        memory,
                        externalFunctions
                    );
                ...
                if (conditionsSatisfied) {
                    const executeInstructionsResult =
                        executeInstructionsWithBreak(
                ...
                while (true) {
                    ...
                    for (const conditionNode of instruction.conditionNodes) {
                        ...
                        const conditionValue =
                            resolveConditionValueResult.value;
                        if (conditionValue.type === "value.true") {
                            conditionsSatisfied = true;
                            continue;
                        }
            ...
            case "instruction.for": {
                const resolveStartValueResult = resolveExpression(
                ...
                const resolveEndValueResult = resolveExpression(
                    instruction.endNode,
                    memory,
                    externalFunctions
                );
                        ...
                        const result: ExecuteInstructionsWithBreakSuccessResult =
                            {
                                ...
                                break: false,
                                returnValue:
                                    executeInstructionsResult.returnValue,
                            };
                        return result;
                    }
                    loopCount100 += 100;
...
type ExecuteInstructionsWithBreakResult =
    | ExecuteInstructionsWithBreakSuccessResult
    | ExecutionErrorResult;
    ...
    for (let i = 0; i < instructionTargetModifierNodes.length; i++) {
        ...
        switch (modifier.type) {
            case "instruction_target_modifier.index_accessor": {
                ...
                if (indexValue.type !== "value.number") {
                    const result: ExecutionErrorResult = {
                        ok: false,
                        startPosition:
                            modifier.indexExpressionNode.startPosition,
            ...
            case "instruction_target_modifier.property_accessor": {
                ...
                const maybeReference =
                    reference.properties.get(modifier.propertyName) ?? null;
...
type GetValueReferenceResult =
    | GetValueReferenceSuccessResult
    | ExecutionErrorResult;
    ...
    switch (expressionNode.type) {
        ...
        case "expression.list_literal": {
            ...
            for (const itemExpressionNode of expressionNode.listItemNodes) {
                const resolveItemResult = resolveExpression(
                    itemExpressionNode,
                    memory,
                    externalFunctions
                );
        ...
        case "expression.object_literal": {
            ...
            for (const [
                propertyName,
                propertyValueExpressionNode,
            ] of expressionNode.objectProperties) {
                const resolvePropertyValueResult = resolveExpression(
                    propertyValueExpressionNode,
                    memory,
                    externalFunctions
                );
        ...
        case "expression.function_call": {
            const externalFunction =
                externalFunctions.get(expressionNode.functionName) ?? null;
            ...
            for (const argumentExpressionNodes of expressionNode.argumentNodes) {
                const resolveArgumentResult = resolveExpression(
        ...
        case "expression.add_operator": {
            const resolveLeftValueResult = resolveExpression(
            ...
            const resolveRightValueResult = resolveExpression(
        ...
        case "expression.multiply_operator": {
            ...
            const value: NumberValue = {
                type: "value.number",
                value100: Math.trunc(
                    (leftValue.value100 * rightValue.value100) / 100
                ),
            };
            const result: ResolveExpressionSuccessResult = {
                ok: true,
        ...
        case "expression.divide_operator": {
            ...
            const value: NumberValue = {
                type: "value.number",
                value100:
                    Math.floor(leftValue.value100 / rightValue.value100) * 100,
            };
            const result: ResolveExpressionSuccessResult = {
                ok: true,
                value: value,
        ...
        case "expression.equal_operator": {
            ...
            if (
                ...
                leftValue.type === "value.true" &&
                rightValue.type === "value.true"
            ) {
                value = {
                    type: "value.true",
                };
            } else if (
                leftValue.type === "value.false" &&
                rightValue.type === "value.false"
            ) {
                value = {
                    type: "value.true",
                };
            } else if (
                leftValue.type === "value.null" &&
                rightValue.type === "value.null"
            ) {
                value = {
                    type: "value.true",
                };
...
type ResolveExpressionResult =
    | ResolveExpressionSuccessResult
    | ExecutionErrorResult;
    ...
    for (const valueAccessorNode of valueAccessorNodes) {
        switch (valueAccessorNode.type) {
            ...
            case "value_accessor.property": {
                ...
                const maybeValue =
                    resultValue.properties.get(
                        valueAccessorNode.propertyName
                    ) ?? null;
                if (maybeValue === null) {
                    const result: ExecutionErrorResult = {
                        ok: false,
...
type GetValueFromValueAccessorsResult =
    | GetValueFromValueAccessorsSuccessResult
    | ExecutionErrorResult;
