...
function init(): void {
	...
	spriteCanvasElement.addEventListener("touchstart", (e) => {
		...
		const target = e.currentTarget as HTMLElement;
		const rect = target.getBoundingClientRect();
		const x = Math.floor((e.touches[0].pageX - target.offsetLeft) / (rect.width / 16));
		const y = Math.floor((e.touches[0].pageY - target.offsetTop) / (rect.height / 16));
		draw(x, y, graphics.colors[colorId]);
	});
...
function createDefaultSprite(): graphics.Sprite {
	const sprite: graphics.Sprite = {
		...
		pixels: new Uint8Array(graphics.spritePixelCount),
	};
	for (let i = 0; i < graphics.spritePixelCount; i++) {
		sprite.pixels[i] = transparencyColorId;
	}
	return sprite;
}
